...
abstract contract CToken is CTokenInterface, ExponentialNoError, TokenErrorReporter {
    ...
    function borrowFresh(address payable borrower, uint borrowAmount) internal {
        ...
        // Calculate BABY discount information for tracking
        ...
        // Emit BABY discount event if discount is applied
        if (borrowRateDiscountBps > 0) {
            emit BabyDiscountApplied(borrower, address(this), baseBorrowRate, actualBorrowRate, borrowRateDiscountBps);
        }
    ...
    function _setBabyStaking(address newBabyStaking) override external returns (uint) { 
        if (msg.sender != admin) {
            revert SetBabyStakingAdminCheck();
        }

        address oldBabyStaking = address(babyStaking);
        babyStaking = newBabyStaking;
