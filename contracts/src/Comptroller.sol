...
/**
 * @title BabyLend's Comptroller Contract
 * @author Compound (Modified)
 * @notice Comptroller without COMP token distribution but with BABY token utility features
 * 
 * ═══════════════════════════════════════════════════════════════════════════════
 * BABY TOKEN UTILITY FEATURES
 * ═══════════════════════════════════════════════════════════════════════════════
 * 
 * This Comptroller implements two core BABY utility features for protocol users who
 * stake BABY tokens:
 * 
 * 1. BORROW RATE DISCOUNTS 
 *    ────────────────────────────────────────────────────────────
 *    Users who stake BABY tokens receive reduced interest rates on borrowed assets.
 *    
 *    Default Tier Structure:
 *    - Tier 0: No stake        → 0% discount
 *    - Tier 1: 10,000 BABY     → 5% discount
 *    - Tier 2: 100,000 BABY    → 10% discount
 *    - Tier 3: 1,000,000 BABY  → 15% discount
 *    - Tier 4: 10,000,000 BABY → 20% discount
 * 
 * 2. LIQUIDATION THRESHOLD BUFFER  
 *    ────────────────────────────────────────────────────────────
 *    BABY stakers require higher shortfall before liquidation can occur.
 ...
 *    - Tier 1: 10,000 BABY     → 2% buffer
 *    - Tier 2: 100,000 BABY    → 3% buffer
 *    - Tier 3: 1,000,000 BABY  → 5% buffer
 *    - Tier 4: 10,000,000 BABY → 7% buffer
 * 
 */
...
contract Comptroller is ComptrollerV8Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    /// @notice Emitted when BABY staking contract is changed
    event NewBabyStaking(address oldBabyStaking, address newBabyStaking);

    /// @notice Emitted when BABY utility features are enabled/disabled
    event BabyUtilityToggled(bool enabled);
    ...
    constructor() {
        admin = msg.sender;
        babyUtilityEnabled = false; // Disabled by default until staking contract is set
    }
    ...
    /*** BABY Utility Functions ***/

    /**
     * @notice Get borrow rate discount for a user based on BABY staking
    ...
    function getBorrowRateDiscount(address borrower) public view override returns (uint) {
        ...
        if (!babyUtilityEnabled || address(babyStaking) == address(0)) {
            return 0;
        }

        // Get discount from staking contract with safety checks
        try babyStaking.getBorrowRateDiscount(borrower) returns (uint discountBps) {
    ...
    /**
     * @notice Get liquidation threshold buffer for a user based on BABY staking
    ...
    function getLiquidationThresholdBuffer(address borrower) public view override returns (uint) {
        ...
        try babyStaking.getLiquidationThresholdBuffer(borrower) returns (uint bufferMantissa) {
    ...
    function _setPendingAdmin(address payable newPendingAdmin) virtual external returns (uint) {
        // Check caller = admin
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_BABY_UTILITY_ADMIN_CHECK);
        }
    ...
    /**
     * @notice Set the BABY staking contract address
     * @dev Admin function to update staking contract reference
     * @param newBabyStaking The address of the new BABY staking contract
     * @return uint 0=success, otherwise a failure
     */
    function _setBabyStaking(address newBabyStaking) external returns (uint) {
        // Check caller is admin
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_BABY_UTILITY_ADMIN_CHECK);
        }
        require(newBabyStaking != address(0), "zero address");
 
        
        address oldBabyStaking = address(babyStaking);
        babyStaking = IBabyStaking(newBabyStaking);
        emit NewBabyStaking(oldBabyStaking, newBabyStaking);
        return uint(Error.NO_ERROR);
    }

    /**
     * @notice Enable or disable BABY utility features globally
     * @dev Admin function to toggle BABY features (emergency kill switch)
     * @param enabled True to enable, false to disable
     * @return uint 0=success, otherwise a failure
     */
    function _setBabyUtilityEnabled(bool enabled) external returns (uint) {
        // Check caller is admin
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_BABY_UTILITY_ADMIN_CHECK);
        }

        babyUtilityEnabled = enabled;

        emit BabyUtilityToggled(enabled);
        return uint(Error.NO_ERROR);
    }
